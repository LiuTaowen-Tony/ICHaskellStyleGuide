...
-- when you can do something less powerful but do the same thing and have the
-- same readability do the less powerful thing
-- e.g. pattern matching < guard < case expression (less versatile)
-- so when can you use pattern match, don't use a guard
-- when can you use a guard, don't use a case expression
...
-- ⬆️ this font will make your code looks super nice
...
-- when precedence is not obvious, use parentheses
-- sometimes it might be obvious for you, but not everyone
...
-------------------------------------------------------------------------------
-- pattern matching
-- pattern matching over guard
-- if you can, try include pre condition and type
-- pre : x >= 0
...
-- if there are more than 2 if clause try use `case` instead, not entirely sure 
-- how to do this, maybe we can ask Nick together
foo :: IO ()
foo = do
  name <- getLine
  case name of
...
-- use list comp
-- when you are mapping and filtering at the same time
oddSquares :: [Integer]
...
-------------------------------------------------------------------------------
-- prefer 'where' over 'let .. in' in pure functions
-- 'let' bindings are commonly used in 'do' blocks
-- but there are some holy wars going on there
-- thus I won't really care which one do you use, as long as it looks good

-- one thing to notice, try avoid have both `let .. in` and `where` in the
-- same function definition

-- for comments, write it before and after function
-- but if it is a super short one, you can put in same line
